...
const ebay_info = () => {
  ...
  const onFinish = async (values) => {
    const newValue = {
      ...
      ebay_date_start: dateData?.ebay_date_start
        ? moment(dateData.ebay_date_start).format("MM-DD-YYYY")
        : "",
      ebay_date_verify: dateData?.ebay_date_verify
  ...
  const getInfoebay = async () => {
    ...
    const newData = {
      ...
      ebay_plan: data.ebay_plan.split(","),
      ebay_processing: data.ebay_processing.split(","),
      ebay_type: data.ebay_type.split(","),
      ebay_sell_status: data.ebay_sell_status.split(","),
      ebay_owner: data.ebay_owner.split(","),
    };
    form.setFieldsValue(newData);
    infoForm.setFieldsValue(newData);
    dateForm.setFieldsValue({
      ebay_date_start: moment(data.ebay_date_start),
      ebay_date_verify: moment(data.ebay_date_verify),
    });
    setInfo(data);
  ...
  const listDate = [
    {
      title: "Ngày tạo",
      value: "ebay_date_start",
    },
    {
      title: "Ngày verify",
      value: "ebay_date_verify",
    },
  ];
            ...
            <Col span={12}>
              <Card title="THÔNG TIN EBAY">
                ...
                >
                  ...
                  <Form.Item label="Quy trình" name="ebay_plan">
                    <Select
                      mode="multiple"
                      style={{ width: "100%" }}
                    ...
                    <Col span={12}>
                      <Form.Item label="Lớp ebay" name="ebay_class">
                        ...
                        >
                          ...
                          <Option value="Lớp 20" label="Lớp 20 ebay error">
                            <div className="demo-option-label-item">
                              Lớp 20 ebay error
                            </div>
                          </Option>
