import {
  ...
  import { getCountCustomer } from "../../api/customer/index";
  import { HuongDanCustomer_class } from "./Customer_list";
  const Customer_class = () => {
    ...
    const columns = [
      {
        ...
        render: (text) => (
          <a
            onClick={() =>
              history.push(`customer_class/table?class=${encodeURIComponent(text)}`)
            }
          >
            {text}
          </a>
      ...
      },
      {
        ...
        content_complete: (
          <div>
            ...
            <br></br>Etsy, customer, shopee, amazone..
          </div>
        ),
        content_check: (
          <div>
      ...
      },
      {
        ...
        content_complete: (
          <div>
            ...
            <br></br>Etsy, customer, shopee, amazone, payoneer, paypal
          </div>
        ),
        content_time: <div>1 ngày</div>,
        content_check: (
      ...
      },
      {
        ...
        content_action: (
          <div>
            ...
            <br></br>Chuẩn bị sản phẩm, customer, card, sim...
    ...
    const countCustomer = async () => {
      let { data } = await getCountCustomer();
      baseData.forEach((item) => {
        data?.data?.forEach((data) => {
          if (data._id == item.class) {
            item.customer_count = data.count.toString();
          }
        });
      });
