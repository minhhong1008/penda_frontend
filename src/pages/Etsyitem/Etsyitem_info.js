import {
  ...
  // Khai báo các kho dữ liệu
    const [etsyitemData, setetsyitemData] = useState({});
  ...
  // Hàm để gửi dữ liệu đi
    const onFinish = async (values) => {
      const newValue = {
        ...info,
        ...values,
        etsyitem_plan: values?.etsyitem_plan ? values.etsyitem_plan.join(",") : "",
        etsyitem_processing: values?.etsyitem_processing
          ? values.etsyitem_processing.join(",")
          : "",
        etsyitem_type: values?.etsyitem_type ? values.etsyitem_type.join(",") : "",
        etsyitem_sell_status: values?.etsyitem_sell_status
          ? values.etsyitem_sell_status.join(",")
          : "",
        etsyitem_owner: values?.etsyitem_owner ? values.etsyitem_owner.join(",") : "",
        etsyitem_employee: values?.etsyitem_employee
          ? values.etsyitem_employee.join(",")
          : "",
      ...
      const response = await updateetsyitemInfo(newValue, id);
  ...
  // Hàm gọi dữ liệu về từ database
    const getInfoetsyitem = async () => {
      const { data } = await getetsyitemInfo(id);
      const newData = {
        ...data,
        etsyitem_plan: data?.etsyitem_plan ?  data.etsyitem_plan.split(","):"",
        etsyitem_employee: data?.etsyitem_employee ?  data.etsyitem_employee.split(","):"",
        etsyitem_processing:data?.etsyitem_processing ?  data.etsyitem_processing.split(","):"",
        etsyitem_type: data?.etsyitem_type ?  data.etsyitem_type.split(","):"",
        etsyitem_sell_status: data?.etsyitem_sell_status ?  data.etsyitem_sell_status.split(","):"",
        etsyitem_owner: data?.etsyitem_owner ?  data.etsyitem_owner.split(","):"",
      };
  ...
  // Hàm để chuyển trang sang các tài khoản khác
    ...
    const listDate = [
      {
        title: "Ngày tạo",
        value: "etsyitem_date_start",
      },
      {
        title: "Ngày verify",
        value: "etsyitem_date_verify",
      },
    ];
                ...
                <Card title="THÔNG TIN ETSY">
                  ...
                  >
                    <Row gutter={16}>
                      ...
                      <Col span={10}>
                        <Form.Item label="etsyitem User" name="etsyitem_user">
                          <Input size="small" placeholder="input here" />
                        </Form.Item>
                      </Col>
                      <Col span={8}>
                        <Form.Item label="etsyitem Pass" name="etsyitem_password">
                          <Input size="small" placeholder="input here" />
                        </Form.Item>
                      </Col>
                    </Row>
                    <Row gutter={16}>
                      <Col span={24}>
                        <Form.Item label="etsyitem chi tiết" name="etsyitem_detail">
                          <Input size="small" placeholder="input here" />
                        </Form.Item>
                      </Col>
                    </Row>
                    ...
                    <Row gutter={16}>
                      ...
                      <Col span={6}>
                        <Form.Item label="etsyitem items" name="etsyitem_item">
                          <Input size="small" placeholder="0" />
                        </Form.Item>
                      </Col>
                      <Col span={6}>
                        <Form.Item label="etsyitem Sold" name="etsyitem_sold">
                          <Input size="small" placeholder="0" />
                        </Form.Item>
                      </Col>
                      <Col span={6}>
                        <Form.Item label="etsyitem Feedback" name="etsyitem_feedback">
                          <Input size="small" placeholder="0" />
                        </Form.Item>
                      </Col>
                    </Row>
  
                    <Form.Item label="Quy trình" name="etsyitem_plan">
                      <Select
                        mode="multiple"
                        style={{ width: "100%" }}
                    ...
                    <Form.Item label="Tiến trình" name="etsyitem_processing">
                      <Select
                        mode="multiple"
                        style={{ width: "100%" }}
                        placeholder="select one item"
                    ...
                    <Form.Item label="Loại etsyitem" name="etsyitem_type">
                      <Select
                        mode="multiple"
                        style={{ width: "100%" }}
                        placeholder="select one item"
                    ...
                    <Form.Item label="TT Bán" name="etsyitem_sell_status">
                      <Select
                        mode="multiple"
                        style={{ width: "100%" }}
                        placeholder="select one item"
                    ...
                    <Form.Item label="Sở hữu" name="etsyitem_owner">
                      <Select
                        mode="multiple"
                        style={{ width: "100%" }}
                        placeholder="select one item"
                    ...
                    <Form.Item label="Nhân viên" name="etsyitem_employee">
                      <Select
                        mode="multiple"
                        style={{ width: "100%" }}
                        placeholder="select one item"
                    ...
                    <Row gutter={16}>
                      <Col span={12}>
                        <Form.Item label="Trạng thái" name="etsyitem_status">
                          <Select
                            //mode="multiple"
                            style={{ width: "100%" }}
                            optionlabelprop="label"
                      ...
                      <Col span={12}>
                        <Form.Item label="Lớp etsyitem" name="etsyitem_class">
                          <Select
                            //mode="multiple"
                            style={{ width: "100%" }}
                            optionlabelprop="label"
                          >
                            ...
                            <Option value="Lớp 20" label="Lớp 20 etsyitem error">
                              <div className="demo-option-label-item">
                                Lớp 20 etsyitem error
                              </div>
                            </Option>
                            <Option value="Lớp 21" label="Lớp 21 etsyitem die">
                              <div className="demo-option-label-item">
                                Lớp 21 etsyitem die
                              </div>
                            </Option>
