...
import { getCountPingpong } from "../../api/pingpong/index";
import { HuongDanPingpong_class } from "./Pingpong_list";
const Pingpong_class = () => {
    ...
    },
    {
      ...
      content_complete: (
        <div>
          ...
          <br></br>Etsy, pingpong, shopee, amazone..
        </div>
      ),
      content_check: (
        <div>
    ...
    },
    {
      ...
      content_complete: (
        <div>
          ...
          <br></br>Etsy, pingpong, shopee, amazone, payoneer, paypal
        </div>
      ),
      content_time: <div>1 ngày</div>,
      content_check: (
  ...
  const countPingpong = async () => {
    let { data } = await getCountPingpong();
    baseData.forEach((item) => {
      data?.data?.forEach((data) => {
        if (data._id == item.class) {
          item.pingpong_count = data.count.toString();
        }
      });
    });
          ...
          <Tabs.TabPane tab="BẢNG TÀI KHOẢN THEO LỚP" key="1">
            <Card type="inner">
              ...
              onRow={(text, rowIndex) => {
                ...
                return {
                  onClick: (event) => {
                    history.push(`pingpong_class/table?class=${encodeURIComponent(text.class)}`);
                  },
                };
              }}
                columns={columns}
