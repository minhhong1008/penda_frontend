...
import { getCountProxy } from "../../api/proxy/index";
import { HuongDanProxy_class } from "./Proxy_list";
const Proxy_class = () => {
  ...
  const columns = [
    {
      ...
      render: (text) => (
        <a
          onClick={() =>
            history.push(`proxy_class/table?class=${encodeURIComponent(text)}`)
          }
        >
          {text}
        </a>
    ...
    },
    {
      ...
      content_complete: (
        <div>
          ...
          <br></br>Proxy, proxy, shopee, amazone..
        </div>
      ),
      content_check: (
        <div>
    ...
    },
    {
      ...
      content_complete: (
        <div>
          ...
          <br></br>Proxy, proxy, shopee, amazone, payoneer, paypal
        </div>
      ),
      content_time: <div>1 ngày</div>,
      content_check: (
  ...
  const countProxy = async () => {
    let { data } = await getCountProxy();
    baseData.forEach((item) => {
      data?.data?.forEach((data) => {
        if (data._id == item.class) {
          item.proxy_count = data.count.toString();
        }
      });
    });
