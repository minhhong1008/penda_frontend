...
import { getCountInfo } from "../../api/info/index";
import { HuongDanInfo_class } from "./Info_list";
const Info_class = () => {
  ...
  const columns = [
    {
      ...
      render: (text) => (
        <a
          onClick={() =>
            history.push(`info_class/table?class=${encodeURIComponent(text)}`)
          }
        >
          {text}
        </a>
    ...
    },
    {
      ...
      content_complete: (
        <div>
          ...
          <br></br>Info, info, shopee, amazone..
        </div>
      ),
      content_check: (
        <div>
    ...
    },
    {
      ...
      content_complete: (
        <div>
          ...
          <br></br>Info, info, shopee, amazone, payoneer, paypal
        </div>
      ),
      content_time: <div>1 ngày</div>,
      content_check: (
  ...
  const countInfo = async () => {
    let { data } = await getCountInfo();
    baseData.forEach((item) => {
      data?.data?.forEach((data) => {
        if (data._id == item.class) {
          item.info_count = data.count.toString();
        }
      });
    });
