...
import { getCountFacebook } from "../../api/facebook/index";
import { HuongDanFacebook_class } from "./Facebook_list";
const Facebook_class = () => {
    ...
    },
    {
      ...
      content_complete: (
        <div>
          ...
          <br></br>Etsy, facebook, shopee, amazone..
        </div>
      ),
      content_check: (
        <div>
    ...
    },
    {
      ...
      content_complete: (
        <div>
          ...
          <br></br>Etsy, facebook, shopee, amazone, payoneer, paypal
        </div>
      ),
      content_time: <div>1 ngày</div>,
      content_check: (
  ...
  const countFacebook = async () => {
    let { data } = await getCountFacebook();
    baseData.forEach((item) => {
      data?.data?.forEach((data) => {
        if (data._id == item.class) {
          item.facebook_count = data.count.toString();
        }
      });
    });
