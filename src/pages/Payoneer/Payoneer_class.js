...
import { getCountPayoneer } from "../../api/payoneer/index";
import { HuongDanPayoneer_class } from "./Payoneer_list";
const Payoneer_class = () => {
  ...
  const columns = [
    {
      ...
      render: (text) => (
        <a
          onClick={() =>
            history.push(`payoneer_class/table?class=${encodeURIComponent(text)}`)
          }
        >
          {text}
        </a>
    ...
    },
    {
      ...
      content_complete: (
        <div>
          ...
          <br></br>Etsy, payoneer, shopee, amazone..
        </div>
      ),
      content_check: (
        <div>
    ...
    },
    {
      ...
      content_complete: (
        <div>
          ...
          <br></br>Etsy, payoneer, shopee, amazone, payoneer, paypal
        </div>
      ),
      content_time: <div>1 ngày</div>,
      content_check: (
  ...
  const countPayoneer = async () => {
    let { data } = await getCountPayoneer();
    baseData.forEach((item) => {
      data?.data?.forEach((data) => {
        if (data._id == item.class) {
          item.payoneer_count = data.count.toString();
        }
      });
    });
