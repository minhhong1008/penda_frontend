...
import { getCountTiktok } from "../../api/tiktok/index";
import { HuongDanTiktok_class } from "./Tiktok_list";
const Tiktok_class = () => {
  ...
  const columns = [
    {
      ...
      render: (text) => (
        <a
          onClick={() =>
            history.push(`tiktok_class/table?class=${encodeURIComponent(text)}`)
          }
        >
          {text}
        </a>
    ...
    },
    {
      ...
      content_complete: (
        <div>
          ...
          <br></br>Etsy, tiktok, shopee, amazone..
        </div>
      ),
      content_check: (
        <div>
    ...
    },
    {
      ...
      content_complete: (
        <div>
          ...
          <br></br>Etsy, tiktok, shopee, amazone, payoneer, paypal
        </div>
      ),
      content_time: <div>1 ngày</div>,
      content_check: (
  ...
  const countTiktok = async () => {
    let { data } = await getCountTiktok();
    baseData.forEach((item) => {
      data?.data?.forEach((data) => {
        if (data._id == item.class) {
          item.tiktok_count = data.count.toString();
        }
      });
    });
