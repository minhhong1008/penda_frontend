...
import { getCountPaypal } from "../../api/paypal/index";
import { HuongDanPaypal_class } from "./Paypal_list";
const Paypal_class = () => {
    ...
    },
    {
      ...
      content_complete: (
        <div>
          ...
          <br></br>Etsy, paypal, shopee, amazone..
        </div>
      ),
      content_check: (
        <div>
    ...
    },
    {
      ...
      content_complete: (
        <div>
          ...
          <br></br>Etsy, paypal, shopee, amazone, payoneer, paypal
        </div>
      ),
      content_time: <div>1 ngày</div>,
      content_check: (
  ...
  const countPaypal = async () => {
    let { data } = await getCountPaypal();
    baseData.forEach((item) => {
      data?.data?.forEach((data) => {
        if (data._id == item.class) {
          item.paypal_count = data.count.toString();
        }
      });
    });
