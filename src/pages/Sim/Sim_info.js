...
// Hàm để gửi dữ liệu đi
  const onFinish = async (values) => {
    const newValue = {
      ...
      sim_date_start: dateData?.sim_date_start
        ? moment(dateData.sim_date_start).format("MM-DD-YYYY")
        : "",
      sim_date_verify: dateData?.sim_date_verify
...
// Hàm gọi dữ liệu về từ database
  const getInfosim = async () => {
    ...
    const newData = {
      ...data,
      sim_employee: data.sim_employee.split(","),
      sim_processing: data.sim_processing.split(","),
      sim_type: data.sim_type.split(","),
      sim_sell_status: data.sim_sell_status.split(","),
      sim_owner: data.sim_owner.split(","),
    };
...
// Hàm để chuyển trang sang các tài khoản khác
  ...
  const listDate = [
    {
      title: "Ngày tạo",
      value: "sim_date_start",
    },
    {
      title: "Ngày verify",
      value: "sim_date_verify",
    },
  ];
                    ...
                    <Col span={12}>
                      <Form.Item label="Lớp sim" name="sim_class">
                        ...
                        >
                          ...
                          <Option value="Lớp 21" label="Lớp 21 sim die">
                            <div className="demo-option-label-item">
                              Lớp 21 sim die
                            </div>
                          </Option>
