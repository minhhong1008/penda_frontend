...
import { getCountBank } from "../../api/bank/index";
import { HuongDanBank_class } from "./Bank_list";
const Bank_class = () => {
  ...
  const columns = [
    {
      ...
      render: (text) => (
        <a
          onClick={() =>
            history.push(`bank_class/table?class=${encodeURIComponent(text)}`)
          }
        >
          {text}
        </a>
    ...
    },
    {
      ...
      content_complete: (
        <div>
          ...
          <br></br>Etsy, bank, shopee, amazone..
        </div>
      ),
      content_check: (
        <div>
    ...
    },
    {
      ...
      content_complete: (
        <div>
          ...
          <br></br>Etsy, bank, shopee, amazone, payoneer, paypal
        </div>
      ),
      content_time: <div>1 ngày</div>,
      content_check: (
  ...
  const countBank = async () => {
    let { data } = await getCountBank();
    baseData.forEach((item) => {
      data?.data?.forEach((data) => {
        if (data._id == item.class) {
          item.bank_count = data.count.toString();
        }
      });
    });
