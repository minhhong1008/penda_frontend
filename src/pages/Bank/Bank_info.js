...
import {
  tablelist_bank_Date,
  listselect_view_acc,
  listselect_bank_plan,
  listselect_bank_block,
  listselect_bank_processing,
  listselect_bank_error,
  listselect_bank_type,
  listselect_bank_sell_status,
  listselect_bank_owner,
  listselect_bank_status,
  listselect_bank_class,
  HuongDanBank_info,
  ContentBank,
} from "./Bank_list";
...
const Bank_info = () => {
  ...
  const [listselect_bank_employee, setListbank_employee] = useState();
  ...
  const submitModalListView = async () => {
    ...
    await updatebankInfo(payload, info.bank_id);
  ...
  const onFinish = async (values) => {
    ...
    const newValue = {
      ...
      bank_note: noteValue,
      bank_history: info.bank_history,
    };
  ...
  const getInfobank = async () => {
    const res = await getbankInfo(id);
    let data = res.data;
    const newData = {
      ...data,
      bank_plan: data?.bank_plan ? data.bank_plan.split(",") : "",
      bank_block: data?.bank_block ? data.bank_block.split(",") : "",
      bank_error: data?.bank_error ? data.bank_error.split(",") : "",
    ...
    setListbank_employee(data.listselect_bank_employee);
  };
  ...
  const onChange_Status = async (values) => {
    if (values == "Error" || values == "Restrict" || values == "Suspended") {
      let new_bank_owner = form.getFieldValue("bank_owner");
      if (new_bank_owner.indexOf("Phòng phục hồi") == -1) {
        new_bank_owner.push("Phòng phục hồi");
      }
      if (new_bank_owner.indexOf("Kho lưu trữ") == -1) {
        new_bank_owner.push("Kho lưu trữ");
      }
      // lưu vào db vì quyền nhân viên không hiển thị
      let { data } = await updatebankInfo(
        {
          bank_owner: new_bank_owner.join(","),
        },
        info.bank_id
      );
      // Tiếp tục set
      let new_bank_processing = form.getFieldValue("bank_processing");
      let old_bank_processing = info.bank_processing;
      if (new_bank_processing.indexOf(values) == -1) {
        new_bank_processing.push(values);
      }

      let new_bank_class = form.getFieldValue("bank_class");
      if (values == "Error") {
        (new_bank_class = "Lớp 20"),
          dateForm.setFieldValue("bankdate_error", moment(now())); // Hiển thị ra màn hình
        dateForm.setFieldValue("bankdate_nextclass", moment(now()));
        setDateData({
          ...dateData,
          bankdate_error: moment(now()),
          bankdate_nextclass: moment(now()),
        }); // Dùng hàm này set lại date mới lưu đc vào db
      }
      if (values == "Restrict") {
        (new_bank_class = "Lớp 23"),
          dateForm.setFieldValue("bankdate_restrict", moment(now()));
        dateForm.setFieldValue("bankdate_nextclass", moment(now()));
        setDateData({
          ...dateData,
          bankdate_restrict: moment(now()),
          bankdate_nextclass: moment(now()),
        });
      }
      if (values == "Suspended") {
        (new_bank_class = "Lớp 26"),
          dateForm.setFieldValue("bankdate_suspended", moment(now()));
        dateForm.setFieldValue("bankdate_nextclass", moment(now()));
        setDateData({
          ...dateData,
          bankdate_suspended: moment(now()),
          bankdate_nextclass: moment(now()),
        });
      }

      form.setFieldsValue({
        bank_class: new_bank_class,
        bank_support: "Nguyễn Hoài",
        bank_processing: new_bank_processing,
  ...
  const onChange_Processing = (values) => {
    if (values[values.length - 1] == "Buyer") {
      ...
      dateForm.setFieldValue("bankdate_start", moment(now()));
      dateForm.setFieldValue("bankdate_nextclass", moment(now()));
      setDateData({
        ...dateData,
        bankdate_start: moment(now()),
        bankdate_nextclass: moment(now()),
      });
    }
    if (values[values.length - 1] == "Verify") {
      ...
      setDateData({
        ...dateData,
        bankdate_verify: moment(now()),
        bankdate_nextclass: moment(now()),
      });
    }
    if (values[values.length - 1] == "Seller") {
      form.setFieldValue("bank_class", "Lớp 9");
      dateForm.setFieldValue("bankdate_seller", moment(now()));
      dateForm.setFieldValue("bankdate_nextclass", moment(now()));
      setDateData({
        ...dateData,
        bankdate_seller: moment(now()),
        bankdate_nextclass: moment(now()),
      });
    }
    if (values[values.length - 1] == "List") {
      form.setFieldValue("bank_class", "Lớp 10");
      dateForm.setFieldValue("bankdate_list1", moment(now()));
      dateForm.setFieldValue("bankdate_nextclass", moment(now()));
      setDateData({
        ...dateData,
        bankdate_list1: moment(now()),
        bankdate_nextclass: moment(now()),
      });
    }
    if (values[values.length - 1] == "Move room") {
      form.setFieldValue("bank_class", "Lớp 12");
      dateForm.setFieldValue("bankdate_moveroom", moment(now()));
      dateForm.setFieldValue("bankdate_nextclass", moment(now()));
      setDateData({
        ...dateData,
        bankdate_moveroom: moment(now()),
        bankdate_nextclass: moment(now()),
      });
    }
  };
  ...
  const onChange_Class = async (values) => {
    ...
    if (values == "Lớp 9") {
      let new_bank_type = form.getFieldValue("bank_type");
      if (new_bank_type.indexOf("Seller") == -1) {
        new_bank_type.push("Seller");
      }

      // lưu vào db vì quyền nhân viên không hiển thị
      let { data } = await updatebankInfo(
        {
          new_bank_type: new_bank_type.join(","),
        },
        info.bank_id
      );

      let new_bank_processing = form.getFieldValue("bank_processing");
      if (new_bank_processing.indexOf("Seller") == -1) {
        new_bank_processing.push("Seller");
      }
    ...
    if (values == "Lớp 4") {
      let new_bank_type = form.getFieldValue("bank_type");
      if (new_bank_type.indexOf("Buyer") == -1) {
        new_bank_type.push("Buyer");
      }
      // lưu vào db vì quyền nhân viên không hiển thị
      let { data } = await updatebankInfo(
        {
      ...
      let new_bank_processing = form.getFieldValue("bank_processing");
      if (new_bank_processing.indexOf("Buyer") == -1) {
        new_bank_processing.push("Buyer");
      }
      /*  let new_bank_owner = form
        .getFieldValue("bank_owner")
        .filter((item) => item !== ""); */
    ...
    if (values == "Lớp 12") {
      let new_bank_type = form.getFieldValue("bank_type");
      if (new_bank_type.indexOf("Bán acc") == -1) {
        new_bank_type.push("Bán acc");
      }
      let new_bank_owner = form.getFieldValue("bank_owner");
      if (new_bank_owner.indexOf("Phòng Kinh doanh") == -1) {
        new_bank_owner.push("Phòng Kinh doanh");
      }
      // lưu vào db vì quyền nhân viên không hiển thị
      let { data } = await updatebankInfo(
        {
          new_bank_type: new_bank_type.join(","),
          new_bank_owner: new_bank_owner.join(","),
        },
        info.bank_id
      );

      let new_bank_processing = form.getFieldValue("bank_processing");
      if (new_bank_processing.indexOf("Move room") == -1) {
        new_bank_processing.push("Move room");
      }
                  ...
                  <Row gutter={16}>
                    <Col span={8}>
                      <Form.Item label="Trạng thái" name="bank_status">
                          ...
                          style={{
                            ...
                            color:
                              ["Suspended", "Error"].indexOf(
                                form.getFieldValue("bank_status")
                              ) != -1
                                ? "red"
                                : "",
                            fontWeight:
                        ...
                        >
                          {listselect_bank_status.map((item, index) => {
                            return (
                              <Option value={item} label={item} key={index}>
                                <div className="demo-option-label-item">
                                  {item}
          ...
          <Row gutter={16}>
            <Col span={12}>
              <Card title="THỜI GIAN: MM-DD-YYYY">
                ...
                >
                  <Row gutter={16}>
                    {tablelist_bank_Date.map((item, index) => {
                      return (
            ...
            <Col span={12}>
              <Card title="LỊCH SỬ">
                ...
                <span>
                  {info?.bank_history?.split(",")?.map((data) => {
                    return <div>{data}</div>;
                  })}
                </span>
              </Card>
